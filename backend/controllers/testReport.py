...
@app.route('/api/project/delateReport', methods=['POST'])
@login_required
def report_delate():
    try:
        report_ids = request.get_json()
        for report_id in report_ids:
            TestReport.update({'_id': ObjectId(report_id)},{'$set': {"isDeleted": True}})
        return jsonify({'status': 'ok', 'data': '删除成功'})
    except BaseException as e:
        return jsonify({'status': 'failed', 'data': '删除失败 %s' % e})
